...
public class PopulationMappingVis {
    ...
    static double grandTotal = 0.00d;
...
public int queryRegion(int x1, int y1, int x2, int y2) {
  if (x1 > x2 || y1 > y2 || x1 < 0 || x2 > width - 1 || y1 < 0 || y2 > height - 1) {
	  System.err.println("Wrong parameter :" + x1+ ","+x2+","+y1+","+y2+","+width+","+height);
    failedQueries = true;
    return -1;
  }
  queryCount++;
...
public double runTest(String test) {
  ...
  long maxTarget = target * total / 100;
  long counted = 0;
  int area = 0;
  boolean[][] count = new boolean[height][width];
  int y = 0;
  int x = 0;
  for (y = 0; y < ret.length; y++)
    for (x = 0; x < ret[y].length(); x++) {
  ...
  if (counted > maxTarget) {
    addFatalError("Selected areas count too much population. counted "+ counted + " maxTarget " + maxTarget);
    return -1;
  }
...
//            try { ImageIO.write(bi, "png", new File("1.png")); } catch (Exception e) {};
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 101; i++) {
          ...
          PopulationMappingVis bw = new PopulationMappingVis(String.valueOf(i));
        }
        System.err.println ("GRAND TOTAL : " + grandTotal+ " Better ?" + (grandTotal >= 542660.9892075199d));
    } 
    // ----------------------------------------------------------------------------------
