 ...
 *
 */
public class PopulationMapping1 {
	...
	public static void main(String[] args) {
		try {
			...
			PopulationMapping1 mapping = new PopulationMapping1();
			String[] ret = mapping.mapPopulation(maxPercentage, worldMap,
					totalPopulation);
	 ...
	 * the grid. For the sake of making pretty maps, I am working on finding
	 * areas of the world with population densities of particular interest.
	 * 
	 * Initially I am given the map of the world's oceans and land in terms of
	 * grid units, and the total population of the world. People live only on
	 * land cells. Because of the wealth of research that various others have
	 * already done, it is possible to determine the population of any
	 * rectangular section of the world (without regard to the various densities
	 * within that region, or the exact population of any cell included in that
	 * region). In your quest to answer this question, you may issue any number
	 * of such queries. Of course, answering each query comes with a cost, and
	 * thus your goal is to balance the accuracy of your answer against the
	 * total number of queries you need to issue.
	 * 
	 ...
	 * 
	 * @param maxPercentage
	 *            gives you the maximum percentage of the total population that
	 *            your selected area may contain.
	 * @param worldMap
	 ...
	 *         must be the same as the input worldMap. Each String contains a
	 *         row of the map. Each cell that you want to select must contain
	 *         the 'X' character, otherwise unselected cells must contain the
	 *         '.' character.
	 * 
	 * 
	 ...
	 * 
	 */
	public String[] mapPopulation(int maxPercentage, String[] worldMap,
			int totalPopulation) {
				...
				if (row[x] == LAND_CHAR) {
					...
					if (!landMap.isEmpty()) {
						...
						for (int i = 0; i < dx.length; i++) {
							...
							if (neighbour_x > -1 && neighbour_y > -1
									&& neighbour_x < width
									&& neighbour_y < height) {
								Point neighbour = new Point(neighbour_x,
										neighbour_y);
								Integer landIndex = landMap.get(neighbour);
								if (landIndex != null && landIndex > -1) {
									found = true;
									landCoordinates.get(landIndex).addPoint(
											landPoint);
		...
		int queryCount = 0;
		for (Land eachLand : landCoordinates) {
			
			// Population.queryRegion(eachLand.min_x,eachLand.min_y,eachLand.max_x,eachLand.max_y);
			int population = queryRegion(eachLand.min_x, eachLand.min_y,
					eachLand.max_x, eachLand.max_y);
			cumulativePopulation += population;
			queryCount++;
			if (population > 0 && cumulativePopulation <= allowedPopulation) {
				markRegion(result_char, eachLand.min_x, eachLand.min_y,
						eachLand.max_x, eachLand.max_y);
				System.err.println(queryCount + ". Population of queryRegion("
						+ eachLand.min_x + "," + eachLand.min_y + ","
						+ eachLand.max_x + "," + eachLand.max_y + ") :"
						+ population + " cumulativePopulation :"
						+ cumulativePopulation + " , allowedPopulation :"
						+ allowedPopulation);
	...
	/**
	 * You can call the queryRegion(int x1, int y1, int x2, int y2) method to
	 * get the population size within a specific rectangular region. (x1,y1)
	 * specifies the lower left corner and (x2,y2) the upper right corner of the
	 * rectangular region. Coordinates are inclusive. A row is specified by y1
	 * and y2. A column is specified by x1 and x2. The method will return an int
	 * containing the population size within the region. The following rules
	 * apply:
	 * 
	...
	static class Land implements Comparable<Land> {
		...
		public int compareTo(Land o) {
			if (point_count > o.point_count) {
				return -1;
			} else if (point_count < o.point_count) {
				return 1;
			}
			return 0;
		}
